...
import { UserScore, UserNetwork, UserProfileResult, User } from 'src/interfaces/user.interface';
...
export class UserRepository implements OnModuleInit {
    ...
    async fecthFriends(username: string, page: number, limit: number): Promise<User[]> {
        ...
        try {
            ...
            return result.records.map(record => record.toObject() as { name: string, createdAt: string });
        } finally {
            await session.close();
        }
    }
